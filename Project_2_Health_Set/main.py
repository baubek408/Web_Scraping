...
iteration_counts = int(len(all_categories))-1
count = 0

print(f"Всего итерации: {iteration_counts}")
...
for category_name, category_href in all_categories.items():
    ...
    # Checking webpage whether there is a table
    alert_block = soup.find(class_="uk-alert-danger")
    if alert_block is not None:
        continue

    # Grabbing all column names with tag <th>
    table_head = soup.find("table", class_="mzr-tc-group-table").find_all("th")
    ...
    if iteration_counts == 0:
        print("Работа завершена")
        break

    print(f"Осталось итерации: {iteration_counts}!")
    sleep(rnd.randrange(2, 4))
